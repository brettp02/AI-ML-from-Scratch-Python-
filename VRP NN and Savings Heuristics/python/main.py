...
def nearest_neighbour_heuristic(px, py, demand, capacity, depot):
    ...
    # Initialize iwth empty set of routes
    routes = []

    while not all(visited):  # while not all nodes are visited
        current_route = []
        current_load = 0
        # Each Route starts from depot
        current_node = depot
        ...
        while True:
            ...
            for i in range(num_nodes):
                if not visited[i] and current_load + demand[i] <= capacity:  # Nearest unvisited node that doesn't exceed capacity
                    distance = utility.calculate_euclidean_distance(px,py,current_node,i)
                    if distance < shortest_distance:
                        shortest_distance = distance
                        next_node = i

            # If no feasible node is found current route is closed and new route started
...
def savings_heuristic(px, py, demand, capacity, depot):
    ...
    # TODO - Implement the Saving Heuristic to generate VRP solutions. - Partly done
    num_nodes = len(px)
    savings = {}
    route_dict = {}
    route_loads = {}
