...
class Neural_Network:
    ...
    def __init__(self, num_inputs, num_hidden, num_outputs, hidden_layer_weights, output_layer_weights, bias_weights, learning_rate):
    ...
    def forward_pass(self, inputs):
        ...
        for i in range(self.num_hidden):
            ...
            weighted_sum = np.dot(inputs, self.hidden_layer_weights[:, i]) + self.bias_weights[i]
        ...
        for i in range(self.num_outputs):
            # TODO! Calculate the weighted sum, and then compute the final output. (COMPLETE)
            weighted_sum = np.dot(hidden_layer_outputs, self.output_layer_weights[:, i]) + self.bias_weights[self.num_hidden + i]
            output = self.sigmoid(weighted_sum)
    ...
    def backward_propagate_error(self, inputs, hidden_layer_outputs, output_layer_outputs, desired_outputs):
        ...
        # Calculate delta_bias_weights
        delta_bias_weights = np.concatenate((hidden_layer_betas, output_layer_betas))
        ...
        return delta_output_layer_weights, delta_hidden_layer_weights, delta_bias_weights

    def update_weights(self, delta_output_layer_weights,delta_hidden_layer_weights, delta_bias_weights):
        ...
        self.bias_weights += self.learning_rate * delta_bias_weights
    ...
    def train(self, instances, desired_outputs, epochs):
        ...
        for epoch in range(epochs):
            ...
            for i, instance in enumerate(instances):
                ...
                delta_output_layer_weights, delta_hidden_layer_weights, delta_bias_weights = self.backward_propagate_error(
                ...
                self.update_weights(delta_output_layer_weights, delta_hidden_layer_weights, delta_bias_weights)
            ...
            acc = np.mean(np.array(predictions) == np.argmax(desired_outputs, axis=1)) # calculates the proportion of correct predictions
    ...
    def predict(self, instances):
        ...
        for instance in instances:
            hidden_layer_outputs, output_layer_outputs = self.forward_pass(instance)
            predicted_class = np.argmax(output_layer_outputs)  # TODO! Should be 0, 1, or 2.(COMPLETE)
            predictions.append(predicted_class)
        return predictions
